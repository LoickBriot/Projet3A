  ...
  def main(args: Array[String]): Unit ={
    ...
    while(true){
      ...
      for (poster <- posterToProcess_List){
        ...
        var updatedRow = posterHandler.newRow(poster.id, posterHandler.getUniqueName(poster), posterHandler.getInputName(poster), text, date, location, price, website, email, phone, true)
